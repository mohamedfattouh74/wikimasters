...
async function main() {
  try {
    ...
    console.log('🔎 Querying existing Neon Auth users from neon_auth.user...');
    const users = await db
      .select({ id: userInNeonAuth.id })
      .from(userInNeonAuth)
      .orderBy(userInNeonAuth.id);

    if (users.length === 0) {
      console.error(
        [
          '❌ No users found in neon_auth.user.',
          'Sign up at least one user through the app first, then re-run db:seed.',
          '(Managed Better Auth owns this table — do not insert users manually.)',
        ].join('\n'),
      );
      process.exit(1);
    }
